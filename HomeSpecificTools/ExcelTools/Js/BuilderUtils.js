...
var tempUnityScriptDir = "./CodeTable/Unity/";
exports.Cleanup = Cleanup = function() {
	Tools.rmDir(tempUnityScriptDir);
	Tools.mkDir(tempUnityScriptDir);
...
exports.buildSpreadsheet = function(gssClassList, outputConfig) {
	...
	if(outputConfig.manTemplateName) {
		...
		fs.writeFileSync(tempUnityScriptDir + outputConfig.manTemplateName + ".cs", content({
	...
	ncp(tempUnityScriptDir, outputConfig.unityScriptDir, function(err) {
		if (err) {
			return console.error(err);
		}
		console.log('复制Unity Script文件至目标路径完成!');
		Cleanup();
	});
...
function gBuildEnumFile(gssClass) {
	...
	for(var i = 0; i < idxKeyList.length; i++) {
		...
		var codeFile = tempUnityScriptDir + "Enum" + className + ".cs";
...
function buildGssClass (gssClass) {
	...
	console.log("export google spreadsheet to cs code and write file: " , gssClass.filename);
	...
	var contentUnity = Handlebars.compile(fs.readFileSync("./Template/UnityTable.template", "utf-8"));
	//class file
	var codeFile = tempUnityScriptDir + "Table" + gssClass.className + ".cs";
	...
	console.log("write to file: " , codeFile);
...
exports.buildJsTemplateClass = function(className, fieldArray, outputConfig) {
	var scriptDir = tempUnityScriptDir + "TableItems/";
