...
program
    .version('0.0.1')
    .usage('<decoSheet-id> <publicdata-id> <unity-script-dir> <json-data-dir> <unity-script-manager-template> <manager-template-name> <matchSheet-id> [options]')
...
var matchSheetId = program.args[6];
var publicdataId = program.args[1];
...
oauth2Client.getAccessToken(function(err, token) {
    if (err) {
        ...
        decoSheet = GoogleSpreadsheet({
            ...
            outputConfig=new Builder.OutputConfig(program.args[2], program.args[3], manTemplate, manTemplateName);
            //console.log('Begin: ', spreadsheet);
            //runOAuth2(spreadsheet);
        running = false;
        
            publicdata = GoogleSpreadsheet({
                key: publicdataId,
                auth: oauth2Client
                ...
                outputConfig=new Builder.OutputConfig(program.args[2], program.args[3], manTemplate, manTemplateName);
                //console.log('Begin: ', publicdata);
                //runOAuth2(publicdata);

                matchData = GoogleSpreadsheet({
                    key: matchSheetId,
                    auth: oauth2Client
                    ...
                    outputConfig=new Builder.OutputConfig(program.args[2], program.args[3], manTemplate, manTemplateName);
                    //console.log('Begin: ', publicdata);
                    for(var i = 0; i < publicdata.worksheets.length; i++)
                    {
                        matchData.worksheets.push(publicdata.worksheets[i]);
                    }
                    for(var i = 0; i < decoSheet.worksheets.length; i++)
                    {
                        matchData.worksheets.push(decoSheet.worksheets[i]);
                    }
