...
program
    .version('0.0.1')
    .usage('<decoSheet-id> <unity-script-dir> <json-data-dir> <build_code> <unity-script-manager-template> <manager-template-name> [options]')
...
var decoSheetId = program.args[0];
var unityScriptDir = program.args[1];
var jsonDataDir = program.args[2];
var outputConfig = null;
var isBuildCode = program.args[3] != 0;
var manTemplate = "";
var manTemplateName = "";
if (program.args.length > 4) {
    manTemplate = program.args[4];
}
if (program.args.length > 5) {
    manTemplateName = program.args[5];
}
var isSheetVertical = program.vertical
var hashJson = program.hash
var listOnly = program.listOnly
var outputConfig = null;
...
oauth2Client.getAccessToken(function(err, token) 
{
    ...
    {
        decoSheet = GoogleSpreadsheet({
            key: decoSheetId,
            auth: oauth2Client
        }, function(err, decoSheet) {
            if (err) {
                throw err;
            }
            outputConfig = new Builder.OutputConfig(unityScriptDir, jsonDataDir, manTemplate, manTemplateName);
...
function readsheetOAuth2(worksheets, idxSheet, countSheet) {
    ...
    worksheets[idxSheet].cells(null, function(err, cells) {
        ...
        //isSheetVertical = true;
        var rowProp = isSheetVertical ? "col" : "row";
        var colProp = isSheetVertical ? "row" : "col";
        ...
        var isHashed = hashJson && !listOnly;
        ...
        rows.forEach(function(col) {
            ...
            col.forEach(function(cell) {
                ...
                var property = properties[cell[colProp]];
                console.log("valueTypes:",valueTypes[property]);
            ...
            if (isHashed) {
                finalList[newObject[hashJson]] = newObject;
            } else {
